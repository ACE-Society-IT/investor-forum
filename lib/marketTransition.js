...
export async function executeGradualMarketShock({
  ...
  const stockSchedules = stocksList.map((stock) => {
    ...
    return {
      ...
      currentTimestamps: Array.isArray(stock.spark_timestamps) ? [...stock.spark_timestamps] : []
    };
  });
  ...
  for (let stepIndex = 0; stepIndex < steps; stepIndex++) {
    ...
    const updatePromises = stockSchedules.map(async (schedule) => {
      ...
      schedule.currentTimestamps = [...schedule.currentTimestamps.slice(-11), nowIso];
      ...
      return supabase
        ...
        .update({
          ...
          spark_timestamps: schedule.currentTimestamps,
          updated_at: nowIso
        })
