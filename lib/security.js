...
// Secure Admin Session Management with Expiry and Hash Check
...
export function createAdminSession(username) {
  ...
  const sessionData = {
    ...
    token: btoa(`${username}:${timestamp}:investor-forum-secure-sig`)
  };
...
export function verifyAdminSession() {
  try {
    ...
    const expectedToken = btoa(`${session.user}:${session.timestamp}:investor-forum-secure-sig`);
